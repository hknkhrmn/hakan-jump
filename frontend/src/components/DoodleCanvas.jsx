...
const DoodleCanvas = ({ onGameOver }) => {
  ...
  useEffect(() => {
    ...
    const drawCharLabel = () => {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(0, GAME_H / 2 - 44);
      ctx.lineTo(GAME_W, GAME_H / 2 - 44);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(0, GAME_H / 2 + 8);
      ctx.lineTo(GAME_W, GAME_H / 2 + 8);
      ctx.stroke();
    ...
    const loop = () => {
      ...
      platforms.forEach((pl) => {
        if (
          ...
          bounceSound
            .play()
            .catch((e) => console.warn("Bounce sesi çalınamadı:", e));
        }
      });
      ...
      if (p.y < 200) {
        ...
        platforms.forEach((pl) => {
          ...
          if (pl.y > GAME_H + 20) {
            Object.assign(
              pl,
              makePlat(20 + Math.random() * (GAME_W - 90), -15),
            );
            pl.broken = 0;
            gs.current.score += 10;
          }
      ...
      if (p.y > GAME_H + 60) {
        if (!gs.current.dead) {
          ...
          fallSound
            .play()
            .catch((e) => console.warn("Fall sesi çalınamadı:", e));
          bgMusic.pause();
    ...
    const handleTouchMove = (e) => {
      ...
      if (gs.current.player.vx > touchMaxSpeed)
        gs.current.player.vx = touchMaxSpeed;
      if (gs.current.player.vx < -touchMaxSpeed)
        gs.current.player.vx = -touchMaxSpeed;
